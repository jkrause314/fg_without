...
static mxArray* do_get_layer_features(const mxArray* const bottom, const mxArray* const layer_name) {
  ...
  for (unsigned int i = 0; i < net_->blobs().size(); ++i) {
    if (net_->blob_names()[i].compare(mxArrayToString(layer_name)) != 0) {
      continue;
    }
    mwSize dims[4] = {net_->blobs()[i]->width(), net_->blobs()[i]->height(),
    net_->blobs()[i]->channels(), net_->blobs()[i]->num()};
    mxArray* mx_blob =  mxCreateNumericArray(4, dims, mxSINGLE_CLASS, mxREAL);
    float* data_ptr = reinterpret_cast<float*>(mxGetPr(mx_blob));
    switch (Caffe::mode()) {
    case Caffe::CPU:
      memcpy(data_ptr, net_->blobs()[i]->cpu_data(),
          sizeof(float) * net_->blobs()[i]->count());
      //caffe_copy(output_blobs[i]->count(), output_blobs[i]->cpu_data(),
      //    data_ptr);
      break;
    case Caffe::GPU:
      //mexPrintf("cudamemcpy\n");
      cudaMemcpy(data_ptr, net_->blobs()[i]->gpu_data(),
          sizeof(float) * net_->blobs()[i]->count(), cudaMemcpyDeviceToHost);
      //mexPrintf("done\n");
      //caffe_copy(output_blobs[i]->count(), output_blobs[i]->gpu_data(),
      //    data_ptr);
      break;
  ...
  LOG(ERROR) << "Invalid layer given";
  mwSize dims[4] = {0,0,0,0};
...
static void get_layer_features(MEX_ARGS) {
  if (nrhs != 2) {
    LOG(ERROR) << "Only given " << nrhs << " arguments";
    mexErrMsgTxt("Wrong number of arguments");
  }
  plhs[0] = do_get_layer_features(prhs[0], prhs[1]);
}
