function res = my_rcnn_test(rcnn_model, imdb, suffix)
...
try
  ...
  for i = 1:num_classes
    fprintf('Trying to load %s bboxes\n', rcnn_model.classes{i});
    load([conf.cache_dir rcnn_model.classes{i} '_boxes_' imdb.name suffix]);
    aboxes{i} = boxes;
  end
  fprintf('Loaded test detections for all classes, no need to recompute\n');
catch
  aboxes = cell(num_classes, 1);
  ...
  % 250 per image, screw it
  max_per_set = 250*length(image_ids);
  max_per_image = 250;
...
for model_ind = 1:num_classes
  ...
  res(model_ind) = imdb.eval_func(cls, aboxes{model_ind}, imdb, conf, suffix);
end
